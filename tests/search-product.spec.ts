...
test('C1ZARA User does not see cookie modal after accepting once', async ({ page }) => {
  ...
  test("Nav1ZARA Should navigate to Zara homepage with correct URL and title", async ({pages,page}) => {
  ...
  test("Search1ZARA: Should find products by name using search ", async ({pages}) => {
    await  pages.searchResultPage.goTo();
    await pages.searchResultPage.goToStore();
